...
export async function POST(request: NextRequest) {
  try {
    ...
    const geminiPrompt = `
      Analyze this patient summary and return concise medical triage guidance.
    ...
    `;
    const geminiResult = await getAIReasoning(geminiPrompt);
    if (geminiResult) {
      try {
        const parsed = JSON.parse(geminiResult.replace(/```json|```/g, "").trim()) as {
          condition?: string;
          insight?: string;
          prevention?: string[];
          specialties?: string[];
        ...
        if (parsed.condition) potentialCondition = parsed.condition;
        if (parsed.insight) aiInsight = parsed.insight;
        if (Array.isArray(parsed.prevention) && parsed.prevention.length > 0) {
          preventionTips = parsed.prevention.slice(0, 3);
        }
        if (Array.isArray(parsed.specialties) && parsed.specialties.length > 0) {
          matchedSpecialties = [...new Set([...matchedSpecialties, ...parsed.specialties])];
        }
      } catch (error) {
        console.warn("Failed to parse Gemini recommendation response:", error);
      }
    }
